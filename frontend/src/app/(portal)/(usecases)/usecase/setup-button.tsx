...
export default function SetupButton({ slug }: { slug: string }) {
    const [link] = useState<string>(`/usecase/${slug}/setup/`);
    const user = useContext(AuthContext);

    const [, SetIsAuth] = useState<boolean>(false);
    ...
    return link ? (
        <Link
            href={link}
            className={` mt-4 cursor-pointer w-full text-center ml-auto bg-orange-500/80  text-white flex-row flex gap-4 justify-center items-center uppercase mx-4 px-4 py-4 ${
                user == undefined ? "bg-gray-500" : "hover:bg-orange-500"
            }`}
        >
