...
const DeviceBox = ({
    ...
    const deleteDevice = useCallback(() => {
        ...
        fetchAPI(
            `/api/thingsboard-plugin/deployment/${setup.id}/${device.id.entityType.split("_")[0].toLowerCase()}/${device.id.id}/delete`,
            {},
            {
                method: "DELETE",
                headers: {
    ...
    return (
        <>
            <div
                className={`bg-gray-500/25 pl-4 pr-2 py-2 flex flex-row items-center ${device.active !== undefined && "border-l-4"} ${device.active ? "border-green-500" : "border-red-500"}`}
            >
...
const ProfileBox = ({ profile, setup, stepData }: { profile: any; setup: any; stepData?: any }) => {
    ...
    const loadDevices = useCallback(() => {
        LoadingState.startLoading();
        fetchAPI(
            `/api/thingsboard-plugin/deployment/${setup.id}/${profile.id.entityType.split("_")[0]}/${profile.id.id}/components`,
            {},
            {
                headers: {
                    Authorization: `Bearer ${Auth.getToken()}`
