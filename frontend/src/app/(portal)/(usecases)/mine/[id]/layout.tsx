...
export default function Layout(props: { children: React.ReactNode; params: { id: number } }) {
    ...
    useEffect(() => {
        ...
        fetchAPI(
            `/api/thingsboard-plugin/deployment/${props.params.id}`,
            {},
            {
                headers: {
            ...
            fetchAPI(
                `/api/thingsboard-plugin/deployment/${props.params.id}/steps/progressComplete`,
                {},
                {
                    headers: {
                        Authorization: `Bearer ${Auth.getToken()}`
    ...
    const deleteUseCase = useCallback(() => {
        LoadingState.startLoading();
        fetchAPI(
            `/api/deployments/${props.params.id}`,
            {},
            {
                method: "DELETE",
                headers: {
    ...
    return (
        <>
            <Suspense>
                ...
                {setup && (
                    <BaseBody>
                        <article className="block rounded bg-white dark:bg-zinc-800 p-6 shadow max-h-full sticky top-0 flex flex-col gap-4">
                            <div className={"flex md:flex-row flex-col gap-8"}>
                                    ...
                                    >
                                        <GalleryImage
                                            thumbnailSrc={
                                                ...
                                                setup.thumbnail.formats.medium.url
                                            }
                                            src={getStrapiURLForFrontend() + setup.thumbnail.url}
                                            alt={""}
                                            caption={setup.thumbnail.caption}
                                            className={
                                ...
                                <div className={"flex flex-shrink flex-col w-full md:w-6/12"}>
                                    <div className={"w-full relative pr-[10.5rem]"}>
                                        ...
                                        <div
                                            className={
                                                "block border-solid border-b-4 pr-2 border-orange-500 "
                                            }
                                        >
                                            <h1
                                                className={
                                                    "dark:text-white font-bold text-3xl capitalize text-pretty break-words hyphens-auto"
                                                }
                                            >
                                                {setup.name}
                                            </h1>
                                    ...
                                    <p
                                        className={
                                            "text-sm text-gray-600 dark:text-gray-200 py-4 flex-grow text-justify"
                                        }
                                    >
                                        {" "}
                                        {setup.description ||
                                            "Eine Zusammenfassung wird kurz um ergänzt."}
                                    </p>
                                </div>
                            </div>
                            <div className={"pb-8"}>
                                <div className={"flex flex-row border-b mb-8 border-gray-300/50"}>
                                    <Suspense>
                                        {setupDevice == true && (
                                            <div
                                                className={
                                                    "text-orange-500 font-extrabold text-xl leading-4"
                                                }
                                            >
                                                <Tab
                                                    name={"Start"}
                                                    link={`/mine/${props.params.id}/start/`}
                                    ...
                                    <Suspense>
                                        {setupDevice == false && (
                                            <Tab
                                                name={"Dashboards"}
                                                link={`/mine/${props.params.id}/dashboards/`}
                                            />
                                        )}
                                    </Suspense>
                                    <Suspense>
                                        {setupDevice == false && (
                                            <Tab
                                                name={"Geräte & Sensoren"}
                                                link={`/mine/${props.params.id}/devices/`}
                                            />
                                        )}
                                    </Suspense>
                                    <Tab
                                        ...
                                        Icon={TrashIcon}
                                        className={
                                            "ml-auto bg-red-500/40 hover:border-b-transparent"
                                        }
                                    ...
                                    {deletePrompt && (
                                        <Prompt
                                            ...
                                            text={
                                                <>
                                                    Diese Aktion ist unwiderruflich. Dabei werden
                                                    alle Verknüpften Daten aus Ihrem Anwendungsfall{" "}
                                                    <b>&quot; {setup.name} &quot;</b> gelöscht. Sind
                                                    Sie sich sicher, dass Sie{" "}
                                                    <b>&quot; {setup.name} &quot;</b> löschen
                                                    wollen?{" "}
                                                </>
                                            }
                                            actions={[
