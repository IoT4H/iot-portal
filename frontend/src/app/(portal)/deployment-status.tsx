...
export const pollStatus = (id: number, setState: Function) => {
    const r = new Promise<boolean>((resolve) => {
        fetchAPI(
            `/api/thingsboard-plugin/deployment/${id}/status`,
            {},
            {
                headers: {
...
export default function Status({ id }: { id: number }) {
    ...
    return state !== State.none ? (
        <span
            ref={ref}
            className={`inline-flex flex-shrink-0 items-center justify-center rounded-md px-2.5 py-1 text-md h-10 gap-0.5 font-medium w-max ${
        ...
        >
            <svg
                className={"h-[1.5em] aspect-square"}
                viewBox="0 0 40 40"
            ...
            {(state === State.created && "Angefordert") ||
                (state === State.deploying && "in Bearbeitung") ||
                (state === State.updating && "Aktualisierung") ||
                (state === State.deployed && "Bereit") ||
                (state === State.failed && "Fehlgeschlagen")}
        </span>
    ) : (
