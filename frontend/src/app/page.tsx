    ...
    return (
        <BaseBody>
            <main className="rounded bg-white dark:bg-zinc-800 p-6 shadow max-h-full pt-8 mt-12 gap-4 block relative">
                <CustomMarkdown className={"markdown mx-8 text-justify"}>
                    {page && page.content}
                </CustomMarkdown>
                <div className={"mt-16 mx-auto w-max h-max relative"}>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                        src={"/undraw_arrow orange.svg"}
                        className={"absolute w-32 -ml-32 -mt-8"}
                    />
                    <Link
                        href={"/usecase"}
                        className={
                            "py-3 px-8 rounded bg-orange-500/20 hover:bg-orange-500/50 cursor-pointer box-border block"
                        }
                    >
                        <strong>Anwendungsfälle</strong> ansehen
                    </Link>
                </div>
            </main>
        </BaseBody>
