...
export default ({ strapi }: { strapi: Strapi }) => ({
  async createTenantForBetrieb(id: number) {
    const firm: any = await strapi.entityService.findOne("api::firm.firm", id, {
  ...
  async createCustomerForBetrieb(id: number) {
    ...
    let updatedFirmState = await strapi.entityService.update(
      "api::firm.firm",
      id,
      {
        data: {
          // @ts-ignore
          CustomerUID: customer.id.i,
        },
      }
    );
    try {
      const customerUser = await strapi
        .plugin("thingsboard-plugin")
        .service("thingsboardService")
        .createCustomerUser(firm.TenentUID, {
          firstName: "Portal",
          lastName: "Default",
          phone: "",
          email:
            createHash("md5")
              .update(customer.id.id + "_" + randomUUID())
      ...
      updatedFirmState = await strapi.entityService.update(
        "api::firm.firm",
        id,
        {
          data: {
  ...
  async deploySetup(deploymentId: number) {
    const deployment: any = await strapi.entityService.update(
    ...
    const useCase: any = await strapi.entityService.findOne(
      "api::use-case.use-case",
      deployment.use_case.id,
      {
        populate: "*",
      }
    );
    ...
    try {
      const allDummies = await Promise.allSettled(
        useCase.components.flatMap((c) =>
          c.Reference.map((r) => {
            return strapi
              .plugin(pluginId)
              .service("thingsboardService")
              .createDummytThingsboardComponentForTenant(
                firm.TenentUID,
                r.entityType.replace(/[^a-zA-Z\d]/gm, "").toLowerCase()
              )
              .then((response) => {
      ...
      const allCopies = await Promise.allSettled(
        useCase.components.flatMap((c) =>
          c.Reference.map((r) => {
            return strapi
              .plugin(pluginId)
              .service("thingsboardService")
              .syncThingsboardComponentForTenant(
                r.id,
                r.tenantId.id,
                refMap.get(r).id,
                firm.TenentUID,
      ...
      const findTemplate = (id: any) => {
        try {
          ...
          const f = fullReference.find((v) => {
            return (
              v.reference.id === id.id &&
              v.reference.entityType === id.entityType
            );
          });
          if (f) {
            return f.template;
      ...
      if (
        constructJson.length !==
        useCase.components.reduce((accumulator, component) => {
          return accumulator + Array.of(...component.Reference).length;
        }, 0)
      ) {
        console.warn(constructJson, useCase.components);
        throw new Error(
          "Not all elements had been properly deployed. Canceling deployment."
        );
      }
  ...
  async createNewDeployment(
    ...
    const deployment: any = await strapi.entityService.create(
      "api::deployment.deployment",
      {
        data: {
  ...
  async getDevicesFromDeployment(deploymentId: number) {
    return this.getComponentsFromDeployment(deploymentId, [
      "ASSET_PROFILE",
      "DEVICE_PROFILE"
    ]);
  },
  async getComponentsFromDeployment(
    deploymentId: number,
    types: string[] = [
      "DASHBOARD",
      "RULE_CHAIN",
    ...
    const comps = (deployment.deployed || [])
      .filter((comp) => {
  ...
  async getComponentProfilesFromSetupProcessOfDeployment(
    deploymentId: number
  ): Promise<any[]> {
    ...
    let profiles = await Promise.all(
      steps
        .filter(
          (steps) => steps.__component === "instructions.setup-instruction"
        )
        .map(async (step) => {
          let t = {
            id: {
              id: step.thingsboard_profile.id,
              entityType: step.thingsboard_profile.entityType
            },
          ...
          return await strapi
            .plugin(pluginId)
            .service("thingsboardService")
            .getThingsboardComponent(t.id.id, t.id.entityType, t.tenantId.id);
        })
    );
  ...
  async getInstructionStepsFromDeployment(deploymentId: number) {
    ...
    const cMs = deployment.use_case.setupSteps.map((step) =>
      step.__component === "instructions.setup-instruction"
        ? this.replaceUUIDsForDeployment(deployment.deployed, step)
        : step
    );
  ...
  async getInstructionStepsProgressCompleteFromDeployment(
    deploymentId: number
  ) {
    const deployment: any = await strapi.entityService.findOne(
      "api::deployment.deployment",
    ...
    if (Array.isArray(deployment.stepStatus)) {
      return {
        complete:
          (deployment.stepStatus.filter((t) => t.progress >= 100) || [])
            .length === deployment.use_case.setupSteps.length,
      };
    } else {
      return { complete: 0 === deployment.use_case.setupSteps.length };
    }
  },
  async updateInstructionStepsProgressFromDeployment(
    deploymentId: number,
    step: {
      __component: string;
      id: number;
      flashInstruction: any[];
      tasks?: any[];
      device: any;
      index: number;
      meta: any;
    ...
    switch (step.__component) {
      case "instructions.setup-instruction":
        const hasflashProcess: boolean =
          Array.of(...step.flashInstruction).length > 0;
        if (Object.keys(subprogress).length === 0) {
          ...
          if (posIndex !== -1) {
            if (hasflashProcess) {
              subprogress.flash = {
                progress:
                  (subprogress.flash && subprogress.flash.progress) ||
                  (currenStatus[posIndex].flash &&
                    currenStatus[posIndex].flash.progress) ||
                  0,
              };
            }
            subprogress.setup = {
              progress:
                (subprogress.setup && subprogress.setup.progress) ||
                (currenStatus[posIndex].setup &&
                  currenStatus[posIndex].setup.progress) ||
                0,
            };
          }
          progress =
            (((subprogress.flash && Number(subprogress.flash.progress)) || 0) +
              ((subprogress.setup && Number(subprogress.setup.progress)) ||
                0)) /
            (hasflashProcess ? 2 : 1);
        }
        break;
      ...
      case "instructions.list-instruction":
        if (Object.keys(subprogress).length === 0) {
          ...
          if (posIndex !== -1) {
            subprogress.tasks = subprogress.tasks.concat(
              currenStatus[posIndex].tasks.filter(
                (f: any) => !subprogress.tasks.some((t) => t.id === f.id)
              )
            );
          }
          progress =
            (subprogress.tasks.filter((t: any) => t.progress === 100).length /
              step.tasks.length) *
            100;
        }
        break;
    ...
    if (posIndex !== -1) {
      currenStatus[posIndex] = Object.assign(currenStatus[posIndex], {
  ...
  async stepAction(
    deploymentId: number,
    data: any,
    ignoreStepsProgressUpdate = false
  ) {
    ...
    return new Promise<void>(async (resolve, reject) => {
      const deployment: any = await strapi.entityService.findOne(
        ...
        deploymentId,
        {
          populate: {
            firm: {
              fields: ["id", "TenentUID", "CustomerUID", "CustomerUserUID"]
            },
            use_case: { populate: "*" }
          },
        }
        ...
        await Promise.allSettled(
          Array.of<{
            ...
            return strapi
              .plugin("thingsboard-plugin")
              .service("thingsboardService")
              .createThingsboardComponentsRelationForTenant(
      ...
      if (
        (
          await this.getInstructionStepsProgressCompleteFromDeployment(
            deploymentId
          )
        ).complete
      ) {
        ...
        case "instructions.setup-instruction":
          if (data.parameter.flash) {
            if (!_ignoreStepsProgressUpdate) {
              returnPromise = strapi
                .plugin("thingsboard-plugin")
                .service("strapiService")
                .updateInstructionStepsProgressFromDeployment(
                  deploymentId,
                  {
                    ...data.step.data,
            ...
            returnPromise = strapi
              ...
              .setupThingsboardDeviceAsset(
                ...
                data.step.data.thingsboard_profile,
                {
                  name:
                    data.parameter.name ||
                    deployment.name + " | " + data.parameter.label,
            ...
            if (!_ignoreStepsProgressUpdate) {
              returnPromise = returnPromise.then(
                ...
                (reason) => {
                  ...
                  strapi.log.warn(
                    `${data.step.data.__component} action failed`
                  );
                  reject(reason);
                }
              );
  ...
  replaceUUIDsForDeployment(
    deployedProfiles: Array<ComponentStructure>,
    objectToReplaceWithin: any
  ) {
    ...
    deployedProfiles.forEach((deployed: ComponentStructure) => {
      ...
      if (deployed.tenantId && deployed.template.tenantId) {
        replacementDictionary[deployed.template.tenantId.id] =
          deployed.tenantId.id;
      }
    });
    return JSON.parse(
      JSON.stringify(objectToReplaceWithin).replace(
  ...
  async deleteAllThingsboardComponentOfDeployment(deploymentId: number) {
    const profiles = (
      await this.getComponentsFromDeployment(deploymentId)
    ).sort((a, b) => {
      const ref = ["dashboard", "assetprofile", "deviceprofile", "rulechain"];
      return (
        ref.indexOf(a.entityType.replace("_", "").toLowerCase()) -
        ref.indexOf(b.entityType.replace("_", "").toLowerCase())
      );
    });
    console.log("profiles:", profiles);
    const DeviceAndAssetProfiles = profiles.filter((p) =>
      ["assetprofile", "deviceprofile"].includes(
        p.entityType.replace("_", "").toLowerCase()
      )
    );

    const te: any = await strapi.entityService.findOne(
      "api::deployment.deployment",
      deploymentId,
    ...
    await Promise.all(
      DeviceAndAssetProfiles.flatMap(async (p: any) => {
        const infos = await strapi
          .plugin("thingsboard-plugin")
          .service("thingsboardService")
          .getThingsboardDevicesInfosOrAssetInfosByProfile(
            tenentUID,
            p.entityType.split("_")[0],
            p.id,
            { page: 0, pageSize: 100 }
          );
        return Promise.all(
          infos.data.map(async (ae: any) => {
            return await strapi
              .plugin("thingsboard-plugin")
              .service("thingsboardService")
              .deleteThingsboardComponentForTenant(
                tenentUID,
                ae.id.entityType.replace("_", "").toLowerCase(),
                ae.id.id
              );
          })
        );
