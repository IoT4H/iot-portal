...
export default ({ strapi }: { strapi: Strapi }) => ({
        ...
        switch (step.__component) {
            ...
            case 'instructions.list-instruction':
                if (Object.keys(subprogress).length === 0) {
                    ...
                    progress =
                        (subprogress.tasks.filter((t: any) => t.progress === 100).length / step.tasks.length) * 100;
                }
                break;
            default:
                break;
    ...
    async existsDeploymentWithName(tenentId: number, deploymentName?: string, setupId?: number) {
