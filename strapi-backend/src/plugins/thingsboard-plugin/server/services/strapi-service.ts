...
export default ({ strapi }: { strapi: Strapi }) => ({
              ...
              .then((response) => {
                ...
                if (response.id.entityType.replace(/[^a-zA-Z\d]/gm, "").toLowerCase() === "dashboard") {
                  strapi
                    .plugin(pluginId)
                    .service("thingsboardService")
                    .assignCustomerToDashboard(firm.TenentUID, firm.CustomerUID, response.id.id)
                    .then(() => {
  ...
  async getComponentsFromDeployment(
    deploymentId: number,
    types: string[] = ["DASHBOARD", "RULE_CHAIN", "ASSET_PROFILE", "DEVICE_PROFILE"]
  ): Promise<any[]> {
  ...
  async getComponentProfilesFromSetupProcessOfDeployment(deploymentId: number): Promise<any[]> {
    ...
    let profiles = await Promise.all(
      steps
        .filter((steps) => steps.__component === "instructions.setup-instruction")
        .map(async (step) => {
          let t = {
  ...
  async getInstructionStepsProgressCompleteFromDeployment(deploymentId: number) {
    ...
    if (Array.isArray(deployment.stepStatus)) {
      return {
        complete:
          (deployment.stepStatus.filter((t) => t.progress >= 100) || []).length ===
          deployment.use_case.setupSteps.length
      };
    } else {
  ...
  async updateInstructionStepsProgressFromDeployment(
    ...
    for (let attribute in step) {
      if (
        !["__component", "id", "flashProcess", "flashInstruction", "tasks", "device", "serverAttributes"].includes(
          attribute
        )
      ) {
    ...
    switch (step.__component) {
      case "instructions.setup-instruction":
        ...
        const hasServerAttributeProcess: boolean =
          Array.of(...step.serverAttributes).filter((sa) => sa.enforced === true).length > 0;
        if (Object.keys(subprogress).length === 0) {
            ...
            if (hasflashProcess) {
              subprogress.flash = {
                progress:
                  ...
                  (currenStatus[posIndex].flash && currenStatus[posIndex].flash.progress) ||
                  0
              };
            }
            if (hasServerAttributeProcess) {
              subprogress.serverAttributes = {
                progress:
                  (subprogress.serverAttributes && subprogress.serverAttributes.progress) ||
                  (currenStatus[posIndex].serverAttributes && currenStatus[posIndex].serverAttributes.progress) ||
                  0
              };
            }
            subprogress.setup = {
              progress:
                ...
                (currenStatus[posIndex].setup && currenStatus[posIndex].setup.progress) ||
                0
            };
          }
          let countOfProcesses = 1;
          if (hasflashProcess) {
            countOfProcesses++;
          }
          if (hasServerAttributeProcess) {
          ...
          progress =
            (((subprogress.flash && Number(subprogress.flash.progress)) || 0) +
              ((subprogress.serverAttributes && Number(subprogress.serverAttributes.progress)) || 0) +
              ((subprogress.setup && Number(subprogress.setup.progress)) || 0)) /
            countOfProcesses;
        }
        break;
        ...
        if (Object.keys(subprogress).length === 0) {
          ...
          if (posIndex !== -1) {
            subprogress.tasks = subprogress.tasks.concat(
              currenStatus[posIndex].tasks.filter((f: any) => !subprogress.tasks.some((t) => t.id === f.id))
            );
          }
    ...
    return new Promise<void>(async (resolve, reject) => {
      ...
      switch (data.step.data.__component) {
        case "instructions.setup-instruction":
          ...
          } else if (data.parameter.serverAttributes) {
            if (!_ignoreStepsProgressUpdate) {
              returnPromise = strapi
                .plugin("thingsboard-plugin")
                .service("strapiService")
  ...
  async deleteAllThingsboardComponentOfDeployment(deploymentId: number) {
    ...
    const DeviceAndAssetProfiles = profiles.filter((p) =>
      ["assetprofile", "deviceprofile"].includes(p.entityType.replace("_", "").toLowerCase())
    );

    const te: any = await strapi.entityService.findOne("api::deployment.deployment", deploymentId, {
    ...
    await Promise.all(
      DeviceAndAssetProfiles.flatMap(async (p: any) => {
        const infos = await strapi
          .plugin("thingsboard-plugin")
          .service("thingsboardService")
          .getThingsboardDevicesInfosOrAssetInfosByProfile(tenentUID, p.entityType.split("_")[0], p.id, {
            page: 0,
            pageSize: 100
          });
        return Promise.all(
    ...
    await Promise.allSettled(
      profiles.map(async (p: any) => {
        return await strapi
          .plugin("thingsboard-plugin")
          .service("thingsboardService")
          .deleteThingsboardComponentForTenant(tenentUID, p.entityType.replace("_", "").toLowerCase(), p.id);
      })
    );
  },
  async existsDeploymentWithName(tenentId: number, deploymentName?: string, useCaseId?: number) {
    console.warn(deploymentName, tenentId);

    const match = await strapi.entityService.findMany("api::deployment.deployment", {
      filters: {
    ...
    if (Array.isArray(deployments) && deployments.length > 0) {
      const newSuggestion = deploymentName.replace(/([0-9]*)\s*$/, numberSuffix);
      if (!deployments.map((d) => d.name).includes(newSuggestion)) {
        suggestions.add(newSuggestion.trim());
      }
    } else {
      suggestions.add(deploymentName.replace(/([0-9]*)\s*$/, numberSuffix).trim());
    }
