...
export default ({ strapi }: { strapi: Strapi }) => {
    ...
    console.log(
        `Connected to ThingsBoard via User: ${strapi
            .plugin(pluginId)
            .config('thingsboardSysAdminUsername')
            .toString()
            .replace(
                /(?<=.).(?=[^@]*?.@)|(?:(?<=@.)|(?!^)\G(?=[^@]*$))(.)(?=.*\.)|(?:(?<=\.)(?=[^.]+$)|(?!^)\G(?=[^@.]*$))[^.](?!$)/gm,
                '*'
    ...
    console.log(
        `Connected to ThingsBoard via Password: ${strapi
            .plugin(pluginId)
            .config('thingsboardSysAdminPassword')
            .toString()
            .replace(/.(?=.{4})/gm, '*')}`
    );
